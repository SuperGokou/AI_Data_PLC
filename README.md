...
<p align="center">
  <a href="https://ai-data-plc-frontend.onrender.com"><strong>Frontend Display Demo</strong></a>
  |
  <a href="https://ai-data-plc-backend.onrender.com/admin/index.html"><strong>Backend Admin Demo</strong></a>
  |
...
当前页面展示的测点与工艺/WIP 数据均来自后端内置的原始表格资源：`backend/src/main/resources/data/ai-points.xls` 与 `backend/src/main/resources/data/full-process.xlsx`。表格未提供的生产批次实时状态、时序曲线和在线告警不做模拟展示。
...
| 后端 API | https://ai-data-plc-backend.onrender.com/api/v1 |
...
- **用户管理** - 后台支持初始化管理员、运行时新增用户、角色分配、启用/停用和删除用户。
- **工业算法层** - 支持后续接入 DeepSeek Pro / GLM 微调或专用工业模型，用于染色结果预测、工艺优化和控制建议。
- **AI 控制安全闸门** - 支持关闭、仅建议、仿真、人工审批、自动下发等控制模式。
- **前端展示端** - React 前端只展示客户可看的工业数据总览、采集链路、工艺流程、测点清单和真实数据来源说明。
- **后端管理台** - Spring Boot 后端托管 `/admin/` 管理界面，负责用户管理、模型/API Provider、API Key 配置和数据接口治理。
...
| 后端管理台 | http://localhost:8080/admin/index.html |
...
| `GET` | `/api/v1/users` | 后台用户列表 |
| `POST` | `/api/v1/users` | 新增或更新后台用户 |
| `PATCH` | `/api/v1/users/{userId}/enabled` | 启用或停用后台用户 |
| `DELETE` | `/api/v1/users/{userId}` | 删除管理员新增的后台用户 |
...
- UI 当前是第一版运营控制台基线，测点和工艺/WIP 数据来自随仓库打包的原始 Excel 表格；生产批次实时状态、时序曲线和在线告警需等待真实数据源接入。
- AI provider 调用和微调任务编排尚未接入外部 API。
- 用户新增 API Provider 与后台用户当前为运行时配置，生产版需要接入数据库和密钥管理服务持久化。
...
The point catalog and process/WIP views are loaded from the bundled source spreadsheets in `backend/src/main/resources/data/ai-points.xls` and `backend/src/main/resources/data/full-process.xlsx`. Batch realtime state, time-series curves, and online alerts are not simulated when the spreadsheets do not provide those records.

### Live Deployment

| Environment | URL |
| --- | --- |
| Frontend display dashboard | https://ai-data-plc-frontend.onrender.com |
| Backend admin console | https://ai-data-plc-backend.onrender.com/admin/index.html |
| Backend API | https://ai-data-plc-backend.onrender.com/api/v1 |
...
- **User management** - Backend console supports bootstrap admins, runtime user creation, role assignment, enable/disable, and user deletion.
...
- **Frontend display dashboard** - The React frontend is reserved for customer-facing display of real spreadsheet-backed overview, acquisition path, process flow, point catalog, and data-source notes.
- **Backend admin console** - The Spring Boot backend hosts `/admin/` for user management, model/API Provider setup, API Key entry, and API contract governance.
...
| Service | URL |
| --- | --- |
| Frontend display dashboard | http://localhost:5173 |
| Backend admin console | http://localhost:8080/admin/index.html |
...
|   `-- Dockerfile
|-- frontend/                 # React 18 + Vite 8 display dashboard
|   |-- src/
|   |   |-- App.tsx           # Display dashboard shell and pages
|   |   |-- main.tsx
|   |   `-- styles.css
|   `-- Dockerfile
|-- deploy/
...
| `GET` | `/api/v1/users` | Backend user list |
| `POST` | `/api/v1/users` | Add or update a backend user |
| `PATCH` | `/api/v1/users/{userId}/enabled` | Enable or disable a backend user |
| `DELETE` | `/api/v1/users/{userId}` | Delete an admin-added backend user |
...
- The UI is a first operations-console baseline. Point and process/WIP data are loaded from the bundled source spreadsheets; batch realtime state, time-series curves, and online alerts require real source data before display.
- AI provider calls and fine-tuning job orchestration are not yet wired to external APIs.
- User-added API providers and backend users are runtime configuration for now; production should persist them through a database and secret manager.
